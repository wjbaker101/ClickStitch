...
public sealed class PatternThreadStitchRepository : Repository<PatternThreadStitchRecord>, IPatternThreadStitchRepository
{
    ...
    {
        var inserts = string.Join(',', stitches.ConvertAll(x => $"({x.Thread.Id},{x.X},{x.Y})"));
        var sql = $"insert into clickstitch.pattern_thread_stitch (pattern_thread_id,x,y) values {inserts};";
